...
curl https://api.stripe.com/v1/events/evt_1NG8Du2eZvKYlo2CUI79vXWy \
  -u "sk_test_51RjOEFRnZtFIzEQoDOfXAtTAZybEeGRK9lHqmaK0LeEWx8wEUeDfopijXDA8s7k4QflB84va3HmhR7weMdQMzvhG009GwxDCpP:"
Response
{
  "id": "evt_1NG8Du2eZvKYlo2CUI79vXWy",
  "object": "event",
...
## Stripe Email Receipts & Customer Linking (Project Note)

- For Stripe to send official email receipts, always pass a valid customer email and use the `customer` field when creating a Checkout session.
- If you only use `customer_email`, Stripe may treat the payment as a "Guest" and not link it to a persistent customer profile.
- Make sure your Stripe dashboard (Settings > Email Receipts) is set to send receipts for successful payments (and for test mode if testing).
- Creating and linking a Stripe customer for every user (and optionally for guests) ensures all receipts, invoices, and future payments are tied to a single customer profile in Stripe.
